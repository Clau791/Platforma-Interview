...
## Ce vrem să livrăm
- Auth cu profiluri (experiență, rol, tehnologii).
- "The Arena": flux voce→STT→LLM→TTS cu real-time emotion context.
- Analiză emoțională (DeepFace/OpenCV) și stocare în PostgreSQL/SQLite.
- Sandbox Docker izolat pentru execuția codului din Monaco Editor.
- Raport JSON final cu secțiuni tehnice, comunicaționale și non-verbale.
- Alegere din UI între OpenAI și Gemini pentru generare LLM.
...
1. **Setup inițial**
   ...
   - Completează variabilele (JWT secret, `OPENAI_API_KEY` sau `GEMINI_API_KEY`, `DATABASE_URL` sau folosești fallback sqlite).
...
- `docs/13-ai-pipeline.md`: flux STT/LLM/TTS + emoții.
- `docs/15-frontend-ui.md`: decizii UI.

## Notă de securitate
- Gestionează cheile în variabile de mediu (OpenAI/Gemini, JWT).
- Fără log-uri de audio/video raw.
- Sandbox-ul Docker este izolat (no network, limite CPU/RAM).
